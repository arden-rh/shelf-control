...
export function toLibraryBook(source: any, manualBook?: boolean): LibraryBookWithoutUserId {
	...
	if (manualBook) {
		industryIdentifier = source.industryIdentifyer;
	} else {
		industryIdentifier = source.industryIdentifiers[0].identifier;
	}
	...
	return {
		...
		industryIdentifyer: industryIdentifier || 'Not available',
